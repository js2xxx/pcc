...
pub fn decompress(data: &[u8], out_len_should: usize) -> LzfResult<Vec<u8>> {
    ...
    while current_offset < in_len {
        ...
        if ctrl < (1 << 5) {
            ...
            let src = &data[current_offset..][..ctrl];
            let dst = &mut output[out_len..][..ctrl];
    ...
    // Set the real length now, user might have passed a bigger buffer in the first
    // place.

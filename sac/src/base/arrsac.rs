//！Copied and modified from github@rust-cv/arrsac.
    ...
    R: RngCore,
{
    ...
    ///
    /// The `inlier_threshold` is the one parameter that is always specific to
    /// your dataset. This must be set to the threshold in which a data
    /// point's residual is considered an inlier. Some of the other
    /// parameters may need to be configured based on the amount of data,
    /// such as `block_size`, `likelihood_ratio_threshold`, and `block_size`.
    /// However, `inlier_threshold` has to be set based on the residual
    /// function used with the model.
    ///
    /// `initial_epsilon` must be higher than `initial_delta`. If you modify
    /// these values, you need to make sure that within one `block_size` the
    /// `likelihood_ratio_threshold` can be reached and a model can be
    /// rejected. Basically, make sure that `((1.0 - delta) / (1.0 -
    /// epsilon))^block_size >>> likelihood_ratio_threshold`. This must be
    /// done to ensure outlier models are rejected during the initial generation
    /// phase, which only processes `block_size` datapoints.
    ///
    /// `initial_epsilon` should also be as large as you can set it where it is
    /// still relatively pessimistic. This is so that we can more easily
    /// reject a model early in the process to compute an updated value for
    /// delta during the adaptive process. This may not be possible and will
    /// depend on your data.
    pub fn new(inlier_threshold: T, rng: R) -> Self {
        Self {
            ...
            block_size: 64,
            likelihood_ratio_threshold: T::from_f32(1e3).unwrap(),
            inlier_threshold,
            rng,
            random_samples: vec![],
        }
    ...
    /// Number of data blocks used to compute the initial estimate of delta and
    /// epsilon before proceeding with regular block processing. This is
    /// used instead of an initial epsilon and delta, which were suggested
    /// by the paper.
    ///
    /// Default: `4`
    #[must_use]
    ...
    /// Once a model reaches this level of unlikelihood, it is rejected. Set
    /// this higher to make it less restrictive, usually at the cost of more
    /// execution time.
    ///
    ...
    ///
    /// This ratio is not exposed as a parameter in the original paper, but is
    /// instead computed recursively for a few iterations. It is roughly
    /// equivalent to the **reciprocal** of the **probability of rejecting a
    /// good model**. You can use that to control the probability
    /// that a good model is rejected.
    ///
    /// Default: `1e3`
    #[must_use]
    pub fn likelihood_ratio_threshold(self, likelihood_ratio_threshold: T) -> Self {
        Self {
    ...
    /// Residual threshold for determining if a data point is an inlier or an
    /// outlier of a model
    #[must_use]
    pub fn inlier_threshold(self, inlier_threshold: T) -> Self {
        Self {
            inlier_threshold,
            ..self
        }
    ...
    R: RngCore,
{
    ...
    /// but it was effectively rewritten to avoid the need for
    /// initial epsilon and delta.
    ///
    /// Returns the initial models (and their num inliers) sorted by decreasing
    /// inliers and `delta` in that order.
        ...
        E: Estimator<Data>,
    {
        ...
        // NOTE: This whole function is different than that specified in the ARRSAC
        // paper. The reason is that you needed to provide a good initial guess
        // for epsilon and delta otherwise it could lead to delta exceeding
        // epsilon or situations where models could no longer be rejected or
        // were almost always rejected. This solution is an imperfect compomise
        // that assumes that delta will be roughly equal to the inlier ratio of the
        // worst model generated, which both assumes that the worst model is an
        // outlier and that it is actually representative of the population. The
        // assumption of epsilon also assumes that the best model is an inlier, but is
        // an otherwise good initial guess. The other caveat with this approach
        // is that a sufficiently large set of initial datapoints is required to
        // be able to accurately determine epsilon and delta. Therefore a new
        // paremeter is added to separate the normal blocks from the initial generation
        // set.
        let mut hypotheses = vec![];
        // We don't want more than `block_size` data points to be used to evaluate
        // models initially.
        let initial_datapoints = core::cmp::min(
            self.initialization_blocks * self.block_size,
            data.clone().count(),
        );
        // Generate the initial batch of random hypotheses and count their inliers and
        // outliers.
        ...
        let initial_datapoints_t = T::from_usize(initial_datapoints).unwrap();
        let epsilon = hypotheses
            .first()
            .map(|&(_, inliers)| T::from_usize(inliers).unwrap() / initial_datapoints_t)
            .unwrap_or_else(T::zero);
        let delta = hypotheses
            .last()
            .map(|&(_, inliers)| {
                T::from_usize(if inliers < E::MIN_SAMPLES {
                    E::MIN_SAMPLES
                } else {
                    inliers
                })
                .unwrap()
                    / initial_datapoints_t
            })
            .unwrap_or_else(T::zero);

        if epsilon < delta {
            // If epsilon is less than delta, then better hypotheses will get rejected and
            // worse accepted, which is counter to what we want. In this case,
            // we had a bad initialization, so clear the hypotheses.
            // This will cause execution to terminate.
            hypotheses.clear();
            return (hypotheses, delta);
        }

        // Populate hypotheses with hypotheses generated from the inliers of the best
        // hypothesis. This will use the initialization datapoints and filter
        // with SPRT.
        ...
        E: Estimator<Data>,
    {
        ...
        // ratios will still be valid (epsilon > delta).
        let epsilon = T::from_usize(hypotheses[0].1).unwrap() / T::from_usize(num_checked).unwrap();
        // Create the likelihood ratios for inliers and outliers.
        let positive_likelihood_ratio = delta / epsilon;
        let negative_likelihood_ratio = (T::one() - delta) / (T::one() - epsilon);
        ...
        if inliers.len() <= E::MIN_SAMPLES {
            // If we don't have enough samples to generate more models, then we should
            // expand the inliers to the entire dataset.
    ...
    /// Generates as many hypotheses as one call to `Estimator::estimate()`
    /// returns from all data.
    ...
    /// Generates as many hypotheses as one call to `Estimator::estimate()`
    /// returns from a subset of the data.
    ...
    /// Algorithm 1 in "Randomized RANSAC with Sequential Probability Ratio
    /// Test".
    ///
    /// This tests if a model is accepted. Returns `Some(inliers)` if accepted
    /// or `None` if rejected.
    ///
    /// `inlier_threshold` - The model residual error threshold between inliers
    /// and outliers `positive_likelihood_ratio` - `δ / ε`
    /// `negative_likelihood_ratio` - `(1 - δ) / (1 - ε)`
    fn asprt<Data, M: Model<Data>>(
        &self,
        data: impl Iterator<Item = Data>,
        model: &M,
        positive_likelihood_ratio: T,
        negative_likelihood_ratio: T,
        minimum_samples: usize,
    ) -> Option<usize> {
        let mut likelihood_ratio = T::one();
        let mut inliers = 0;
        for data in data {
            likelihood_ratio *=
                if T::from_f64(model.residual(&data)).unwrap() < self.inlier_threshold {
                    inliers += 1;
                    positive_likelihood_ratio
                } else {
                    negative_likelihood_ratio
    ...
    R: RngCore,
{
        ...
        I: Iterator<Item = Data> + Clone,
    {
        ...
        // Generate the initial set of hypotheses. This also gets us an estimate of
        // delta.
        ...
        // Gradually increase how many datapoints we are evaluating until we evaluate
        // them all. This starts at the first block that was not evaluated in
        // initial_hypotheses.
        'outer: for block in self.initialization_blocks.. {
            ...
            for sample in samples_up_to_beginning_of_block..samples_up_to_end_of_block {
                ...
                for (hypothesis, inlier_count) in hypotheses.iter_mut() {
                    if T::from_f64(hypothesis.residual(&new_datapoint)).unwrap()
            ...
            // this basic right shift below, but as written it contained some apparent
            // errors in where it was ran. This seems to be the correct location
            // to do this.
            hypotheses.sort_unstable_by_key(|&(_, inliers)| Reverse(inliers));
